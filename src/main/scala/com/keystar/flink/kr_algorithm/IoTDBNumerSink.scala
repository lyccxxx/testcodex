...
class IoTDBNumerSink extends KeyedProcessFunction[String, AlgorithResult, Unit] {

  private var buffer: ListState[AlgorithResult] = _
  ...
  override def open(parameters: Configuration): Unit = {
    val stateDescriptor = new ListStateDescriptor[AlgorithResult]("resultState", classOf[AlgorithResult])
  ...
  override def processElement(value: AlgorithResult, ctx: KeyedProcessFunction[String, AlgorithResult, Unit]#Context, out: Collector[Unit]): Unit = {
    //    println(s"Data received in subtask ${Thread.currentThread().getId}: $value")
    if(value.collectedValue.exists(_!=null)||value.collectedValue.exists(_!=None)){
      buffer.add(value)
    }
...
//      println(s"输出长度：${currentCount}")
  ...
  override def onTimer(timestamp: Long, ctx: KeyedProcessFunction[String, AlgorithResult, Unit]#OnTimerContext, out: Collector[Unit]): Unit = {
    val currentTime = ctx.timerService().currentProcessingTime()
...
//      logger.info(s"Processed data 数据结束时间")
  ...
  private def getBatchFromBuffer: List[AlgorithResult] = {
    buffer.get().iterator().asScala.toList
  }

  def flushBatch(): Unit = {
    var jsonString: String = null
    var batch: List[AlgorithResult] = null
...
//      logger.info(s"打印json:${jsonString}")
  ...
  def OrganizeToData(data: List[AlgorithResult]): String = {
    if (data.isEmpty) return ""

    data.foreach { result =>
      // 检查表达式是否为目标值
      if (result.expressions == "A2_07_isPowerFactorLow") {
        // 打印该结果的详细信息
        println(s"找到A2_07_isPowerFactorLow记录:")
        println(s"  时间戳: ${result.timestamp}")
        println(s"  测点: ${result.expressions}")
        println(s"  计算值: ${result.collectedValue.getOrElse("无值")}")
        println(s"  告警状态: ${if (result.alarm) "告警中" else "正常"}")
        println(s"  设备标签: ${result.equip_label}")
        println(s"  算法ID: ${result.algorithm_id}")
        println(s"  站点: ${result.station}")
        println("------------------------")
      }
    }
